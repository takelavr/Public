    ...
    public String calculate(@Param("method") String method,
        ...
        if (cityFromId == cityToId) {
            ...
            switch (method) {
                ...
                case ("datamatrix"): {
                    return  new DistanceCalculate().calculateDataMatrix(cityRepo.findById(cityFromId).get(),
                                                                   cityRepo.findById(cityToId).get());
                }
                case ("all"): {
                    return  "DataMantix " + new DistanceCalculate().calculateDataMatrix(cityRepo.findById(cityFromId).get(),
                                                                                                    cityRepo.findById(cityToId).get())
