...
public class ApiController {
	...
	public ApiController(JpaShaurmaRepository shaurmaRepo, JpaOrderRepository repo) {
...
//	}
	...
	//Заказы
	//получение заказа по Ig (GET)
	public Order getOrderById (String orderId) {
		return rest.getForObject("http://localhost:8080/orders/{id}", Order.class, orderId);
	}
	
	//добавление заказа POST
	public Order createOrder(Order order) {
		return rest.postForObject("http://localhost:8080/orders/{id}", order, Order.class);
	}
	...
	//правка заказа PUT (полная замена заказа)
	 @PutMapping(path="/{orderId}", consumes="application/json")
	  public Order putOrder(@RequestBody Order order, JpaOrderRepository repo) {
